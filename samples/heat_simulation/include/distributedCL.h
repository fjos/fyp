...
class distributedCL
{
  public:
    ...
    template <typename data_type>
    void BroadcastValue(int data_size, data_type *data, int source_machine)
    {
    ...
    cl_int EnqueueBarrier(cl_command_queue command_queue, init_list target_machines);
        ...
        if (world_rank_in_list(target_machines, world_rank))
        {
            if (recv_event != NULL)
            {
                ...
                err = clEnqueueWriteBuffer(command_queue, buffer, blocking_write,
                                           ...
                                           0, NULL, recv_event->events[0].get());
            }
            else
            {
                err = clEnqueueWriteBuffer(command_queue, buffer, blocking_write,
...
cl_int distributedCL::EnqueueBarrier(cl_command_queue command_queue, init_list target_machines)
{
    cl_int err;
    if (world_rank_in_list(target_machines, world_rank) ||
        target_machines.size() == 0)
